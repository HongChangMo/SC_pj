        ...
        <div class="col-12 content-wrap">
            <section>
                <div class="breadcrumb">
                    ...
                    &gt;
                    <a href="/knowledge/knowledgeList">지식</a>
                    &gt;
                    <!-- 카테고리에 맞춰 변경해줘야함 -->
                    <a class="active" href="/knowledge/knowledgeWrite">new 지식</a>
                </div>
                <form id="sendForm" name="sendForm" method="post">
                <input type="hidden" name="knContents" />
                <input type="hidden" name="knWriter" value="${UserDTO.userId}" />
                <div class="content-info">
                    <div class="content-topic col-12">
                        <label for="topic">토픽</label>
                        <select id="topic" name="knTopic" class="sel-default">
                    ...
                    <div class="content-title col-12">
                        ...
                        <input id="input_title" class="form-control" name="knTitle"/>
                    </div>
                    <!-- 해시태그 영역 start -->
...
<script>
    ...
    $("#submitBtn").on('click', function() {
        ...
        const topic = $('form[name="sendForm"]').find('select[name="knTopic"]').val();
        const title = $('form[name="sendForm"]').find('input[name="knTitle"]').val();
        ...
        if( topic == '' || topic == null ) {
            alert( "토픽을 선택해주세요" );
            $('form[name="sendForm"]').find('select[name="knTopic"]').focus();
            return false;
        } else if( title == '' || title == null ) {
            alert("제목을 입력해주세요");
            $('form[name="sendForm"]').find('input[name="knTitle"]').focus();
            return false;
        } else if( hashtags < 1 ) {
            alert("태그를 선택해주세요");
            return false;
            ...
            alert("본문을 입력해주세요");
            $('form[name="sendForm"]').find('textarea[name="knContents"]').focus();
            return false;
    ...
    function jsQnaAdd(contents) {
        ...
        $("#sendForm").find('input[name="knContents"]').val(contents);

        let form = $("#sendForm").serialize();

        $.ajax({
            type: 'post',
            url: '/knowledge/knowledgeAdd',
            dataType: 'json',
            data: form,
            success: function(ajaxResult) {
                alert("등록되었습니다.");
                // view 페이지 개발 완료시 view 페이지로 이동
                location.href='/knowledge/knowledgeList';
            },
            error: function(err) {
                alert("등록에 실패했습니다.");
            }
